...
<div id="add-precache">
	...
	<input type="text" id="new-precache" size="50"/>
	<button class="button-primary" onclick="addPrecache()">Add Precache</button>
</div>
...
<div id="preaches-container">
	<form method="POST">
		...
		<p class="description">Check files below that you want to enable as precaches assets. Unchecked files will not
			precaches.</p>
		<ul id="precaches-list">
			<li><input type="checkbox" value="select_all" onclick="selectAllCaches(this)"/> <strong
					style="font-size:1.2em;">Select All Preaches</strong></li>
			<?php
			$precaches = MCW_PWA_Service_Worker::instance()->getPrecaches();
			foreach ( $precaches as $key => $precache ) :
				?>
				<li>
					<input type="checkbox" name="mcw_precaches[]" class="precaches" value="<?php echo $precache; ?>"
					       checked/> <?php echo $precache; ?> <a href="javascript:void(0);" onclick="removeAsset(this)">Remove</a>
			...
			if ( is_array( $assets ) ) :
				foreach ( $assets as $key => $precache ) :
					?>
					<li>
						<input type="checkbox" name="mcw_precaches[]" class="precaches"
						       value="<?php echo $precache; ?>"/> <?php echo $precache; ?> <a href="javascript:void(0);"
						                                                                      onclick="removeAsset(this)">Remove</a>
						<input type="hidden" name="mcw_assets[]" class="assets" value="<?php echo $precache; ?>"/>
					</li>
				<?php endforeach; ?>
			<?php endif; ?>
